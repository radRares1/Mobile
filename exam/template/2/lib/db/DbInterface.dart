...
class DbInterface<T extends ObjectInterface> {
  Database _db;
  String _table;
  Map<String, String> _cols;
  T _data;

  DbInterface(String table, Map<String, String> cols, T data){
    this._table = table;
    this._cols = cols;
    this._data = data;
  }
    ...
    var d = await openDatabase(path, version: 1,
        onCreate: (Database db, int version) async {
          ...
          _cols.forEach((k, v) => (k == 'id'
              ...
              : dbCreateString += k + " " + v + ",\n"));
          dbCreateString = dbCreateString.substring(0, dbCreateString.length - 2);
          dbCreateString += ")";
  ...
  Future<int> update(int id, ObjectInterface obj) async {
    var dbClient = await db;
