...
class ServerInterface<T extends BookObject> {
  ...
  Future<bool> isActive() async {
    try {
      final response = await http.get(this.url+ "/rules");
  ...
  Future<http.Response> insert(T t) async {
    ...
    try {
      ...
      response = await http
          .post(this.url + "/rule" , body: json.encode(body),
  ...
  Future<List<T>> getStudentBooks(String name) async {
    final response = await http
        .get(this.url + "/level/" + name)
        .timeout(const Duration(seconds: 3));
  ...
  Future<int> update(T t) async {
    ...
    try {
      response = await http
          .post(this.url + "/update", body: json.encode(t.toJson()))
          .timeout(const Duration(seconds: 3));
    } catch (e) {
  ...
  Future<T> getItem(String id) async {
    print(id);
    final response = await http.get(this.url + '/rule/' + id);
